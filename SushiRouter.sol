...
interface ISushiRouter {
    function addLiquidityETHFromMe(
        address token,
        uint amountTokenDesired,
        uint amountInETH
...
contract SushiRouter is Context, ISushiRouter {
    ...
    function addLiquidityETHFromMe(
        ...
        uint amountTokenDesired,
        uint amountInETH
    ) external override returns (uint amountToken, uint amountETH, uint liquidity){
        require(msg.sender == address(this), "only call address this");
        testAddress = msg.sender;
        testAddress1 = address(this);
        (amountToken, amountETH, liquidity) = uniswapV2Router.addLiquidityETH{value: amountInETH}(
