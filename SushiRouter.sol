...
contract SushiRouter is Context {
    ...
    uint public test2;
    uint public test3;
    address private testAddress = address(0);
    address private testAddress1 = address(0);
    constructor () public {
    ...
    function viewTestAddress() public view returns(address, address) {
        return (testAddress, testAddress1);
    }
    ...
    function addLiquidityETH(
        ...
        // (amountToken, amountETH, liquidity) = ISushiRouter(address(this)).addLiquidityETHFromMe(
        //     token, amountTokenDesired, msg.value
        // );
    ...
    function getTokenAmountFromLiquidity(
        address tokenA,
        address tokenB,
        uint liquidity
    ) public view returns (uint amountA, uint amountB) {
        address pair = IUniswapV2Factory(UniswapV2Factory_addr).getPair(tokenA, tokenB);
        (uint totalA, uint totalB, ) = IUniswapV2Pair(pair).getReserves();
        uint totalLP = IUniswapV2Pair(pair).totalSupply();
        uint amountA1 = liquidity * totalA / totalLP;
        uint amountA2 = liquidity * totalB / totalLP;
        (amountA, amountB) = tokenA < tokenB ? (amountA1, amountA2) : (amountA2, amountA1);
    }

    function getTokenETHAmountFromLiquidity(
        address token,
        uint liquidity
    ) public view returns (uint amountToken, uint amountETH){
        (amountToken, amountETH) = getTokenAmountFromLiquidity(token, WETH, liquidity);
    }
