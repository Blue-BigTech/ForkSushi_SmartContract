...
interface IStakingPlatform {
     ...
     * and `endPeriod` which is `startPeriod` + `stakingPeriodUnit`
     */
    function initialStartTime() external;
     ...
     */
    event InitializedStartTime(uint startTime);
}
...
contract StakingPlatform is IStakingPlatform, Ownable {
    ...
    uint public fixedAPY = 133680000000000; //APY 5% daily
    // uint public fixedAPY = 1546098755; //APY 5% secondly
    ...
    mapping(address => uint) private _userLockTime;
    ...
    constructor(
        address _token,
        uint _fixedAPY,
        uint _lockDurationInSeconds
    ) public {
        token = IERC20(_token);
        fixedAPY = _fixedAPY;
        lockupDuration = _lockDurationInSeconds * 1 seconds;
    ...
    function changeStakingPeriodUnit(uint _periodInSeconds) public onlyOwner {
      stakingPeriodUnit = _periodInSeconds * 1 seconds;
    }

    function changeLockupDuration(uint _lockDurationInSeconds) public onlyOwner {
      lockupDuration = _lockDurationInSeconds * 1 seconds;
    }
    /**
     * @notice function that start the staking
     ...
     * and `endPeriod` which is `startPeriod` + `stakingPeriodUnit`
     */
    function initialStartTime() external override onlyOwner {
        stakingStartTime = block.timestamp;
        emit InitializedStartTime(stakingStartTime);
    }

    modifier checkLockupPeriod() {
        require(
            _userLockTime[_msgSender()] <= block.timestamp,
            "Locking period"
        );
        _;
    }
     ...
     * @dev that the amount deposited should greater than 0
     */
    function deposit(uint amount) external override checkLockupPeriod{
        require(amount > 0, "Amount must be greater than 0");
        _updateRewards();

        _userStartTime[_msgSender()] = block.timestamp;
        _userLockTime[_msgSender()] = block.timestamp + lockupDuration;
        ...
        returns (uint)
    {
        ...
        return  (calcReward(staked[stakeHolder], fixedAPY, _pastTimeDuration(stakeHolder)).add(_rewardsToClaim[stakeHolder]));
    }
     ...
     */
    function _pastTimeDuration(address stakeHolder)
        internal
        view
        returns (uint)
    {
        uint startTime = stakingStartTime >= _userStartTime[stakeHolder] ? stakingStartTime : _userStartTime[stakeHolder];
        return (block.timestamp - startTime) / stakingPeriodUnit;
    }
    ...
    function calcReward (uint principal, uint ratio, uint n) public pure returns (uint) {
     ...
     * if rewards to claim
     */
    function withdraw(uint amount) external override checkLockupPeriod{
        require(amount > 0, "Amount must be greater than 0");
        require(
        ...
        staked[_msgSender()] = staked[_msgSender()].sub(amount);
     ...
     * if rewards to claim
     */
    function withdrawAll() external override checkLockupPeriod{
        _updateRewards();
        if (_rewardsToClaim[_msgSender()] > 0) {
            _claimRewards();
        }
        ...
        _userLockTime[_msgSender()] = 0;
        _totalStaked = _totalStaked.sub(staked[_msgSender()]);
    ...
    function _claimRewards() private {
        ...
        token.transferFrom(owner(), _msgSender(), rewardsToClaim);
    ...
    function withdrawResidualBalance() external onlyOwner {
        require(
            block.timestamp >= stakingStartTime + (365 * 1 days),
            "Withdraw 1 year after start staking"
        );
     ...
     * @dev transfer the pending rewards to the `msg.sender`
     */
    function claimRewards() external override checkLockupPeriod{
        _claimRewards();
        _userLockTime[_msgSender()] = block.timestamp + lockupDuration;
    }
}